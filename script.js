...
async function loadJlptKanjiFile(){try{const response=await fetch("data/jlpt-kanji.json",{cache:"no-store"});if(!response.ok)throw new Error("local kanji file unavailable");const data=await response.json();["n5","n4","n3"].forEach(level=>{if(Array.isArray(data[level]))jlptDeck[level]=mergeKanjiDeck([...data[level],...(jlptDeck[level]||[])])})}catch(error){}}
...
function getKanjiDifficulty(card=currentQuestion){const strokes=getKanjiStrokeCount(card);if(trainerMode==="n3"||strokes>=14)return"Hard";if(trainerMode==="n4"||strokes>=9)return"Medium";return"Starter"}
function updateJlptDashboard(){const next=progress.level*100,learned=Math.min(jlptCorrectCount,30),hp=Math.max(0,Math.min(100,jlptBossState.hp)),phase=jlptAdventure.phase||"learning";const set=(id,text)=>{const el=document.getElementById(id);if(el)el.textContent=text};set("jlptStreakDash",progress.streak||0);set("jlptLearnedDash",`${learned}/30`);set("jlptYenDash",progress.yen||0);set("jlptExpDash",`${progress.exp||0}/${next}`);set("jlptBossName",getCurrentBossName());set("jlptBossHpText",`Boss HP: ${hp}%`);set("jlptWorldLabel",getCurrentWorld());set("jlptPhaseTitle",phase==="boss"?"Boss Challenge":phase==="review"?"Review Session":"Learning Phase");set("jlptSetText",phase==="review"?`Review ${jlptAdventure.reviewCorrect||0}/3 correct`:`${Math.min(jlptAdventure.learnedSet||0,5)}/5 cards learned`);const fill=document.getElementById("jlptBossFill");if(fill)fill.style.width=hp+"%";const setFill=document.getElementById("jlptSetFill");if(setFill)setFill.style.width=(phase==="review"?Math.min((jlptAdventure.reviewCorrect||0)/3*100,100):Math.min((jlptAdventure.learnedSet||0)/5*100,100))+"%";document.getElementById("jlptBossCard")?.classList.toggle("hidden",!jlptAdventure.bossReady&&phase!=="boss");document.getElementById("jlptBossButton")?.classList.toggle("hidden",phase==="boss");document.getElementById("jlptAchFirst")?.classList.toggle("unlocked",jlptCorrectCount>=1);document.getElementById("jlptAchScholar")?.classList.toggle("unlocked",jlptCorrectCount>=5);document.getElementById("jlptAchShrine")?.classList.toggle("unlocked",jlptCorrectCount>=10);document.getElementById("jlptAchMountain")?.classList.toggle("unlocked",trainerMode==="n3"&&jlptCorrectCount>=15);document.getElementById("jlptAchBoss")?.classList.toggle("unlocked",(jlptBossState.defeated||0)>=1);document.getElementById("jlptAchMaster")?.classList.toggle("unlocked",jlptCorrectCount>=30);document.querySelectorAll(".jlpt-journey-row span").forEach((item,index)=>item.classList.toggle("active",index===(phase==="learning"?0:phase==="review"?1:phase==="event"?2:phase==="boss"?3:4)))}
...
function applySmallJlptReward(yenGain=2,expGain=4){if(jlptAdventure.xpBoost>0){expGain+=Math.ceil(expGain*.5);jlptAdventure.xpBoost--}progress.triedLearning=true;progress.yen=Number(progress.yen||0)+yenGain;progress.points=Number(progress.points||0)+expGain;progress.exp=Number(progress.exp||0)+expGain;progress.level=Number(progress.level||1);let next=progress.level*100;while(progress.exp>=next){progress.exp-=next;progress.level++;next=progress.level*100;progress.yen+=25}saveProgress();updateLeaderboard();updateTrainerUI();pulseHudElement("hudYen");pulseHudElement("hudExpFill");return{yenGain,expGain}}
...
function nextJlptQuestion(){lockAnswer=false;hintUsedThisQuestion=false;if(jlptAdventure.phase==="event"||jlptAdventure.phase==="rewards")jlptAdventure.phase="learning";hideKanjiExpansion();const pool=getQuestionPool(),phase=jlptAdventure.phase||"learning",bossReady=phase==="boss";let picked=pool[Math.floor(Math.random()*pool.length)];if(pool.length>1){let guard=0;while(picked.q===lastJlptQuestionKey&&guard<8){picked=pool[Math.floor(Math.random()*pool.length)];guard++}}lastJlptQuestionKey=picked.q;currentQuestion={...picked,boss:bossReady};const reviewSetup=phase==="review"?getReviewQuestionSetup(currentQuestion,pool):{answer:currentQuestion.a,question:"Choose the correct meaning.",answers:createAnswers(pool,currentQuestion.a)};currentQuestion.answer=reviewSetup.answer;const level=getJlptLevelLabel();document.getElementById("jlptModeLabel").textContent=`JLPT ${level}`;document.getElementById("jlptRoundLabel").textContent=phase==="learning"?"Learning card":phase==="review"?"Review quiz":bossReady?"Boss challenge":"Kanji card";document.getElementById("jlptPrompt").textContent=decodeMojibakeText(currentQuestion.q);document.getElementById("jlptQuestion").textContent=phase==="learning"?"Study the meaning, readings, and examples.":trainerMode==="words"?"Choose the correct meaning.":bossReady?"Boss challenge: correct answers deal damage.":reviewSetup.question;updateKanjiMetaBadges(currentQuestion);const meta=document.getElementById("kanjiMeta"),info=document.getElementById("kanjiInfo"),tipBtn=document.querySelector(".kanji-info-btn"),learnBtn=document.getElementById("jlptLearnBtn"),grid=document.getElementById("jlptAnswerGrid");if(meta){meta.classList.add("locked");meta.textContent=jlptAdventure.freeHints>0?"💡 Free hint available":"💡 Hint available · costs 3 YEN and 5 EXP"}if(info){info.textContent="";info.classList.add("hidden")}if(tipBtn)tipBtn.textContent="💡 Hint";if(learnBtn)learnBtn.classList.toggle("hidden",phase!=="learning");grid.classList.toggle("hidden",phase==="learning");document.getElementById("jlptQuizView").classList.toggle("boss-mode",bossReady);if(phase==="learning"){renderKanjiExpansion(currentQuestion);document.getElementById("jlptFeedback").textContent="Learn 5 cards to unlock a short review session.";grid.innerHTML=""}else{document.getElementById("jlptFeedback").textContent=bossReady?"Defeat the boss with correct answers.":"Review time: answer 3 correctly to continue your journey.";grid.innerHTML="";reviewSetup.answers.forEach(answer=>{const btn=document.createElement("button");btn.className="answer-btn premium-answer";btn.textContent=answer;btn.onclick=()=>checkAnswer(btn,answer);grid.appendChild(btn)})}updateJlptDashboard();enrichKanjiFromApis(currentQuestion)}
