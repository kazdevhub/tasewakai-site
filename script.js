...
const storeItems=[
  {id:"sushi",name:"Sushi Set",price:40,img:"assets/store-sushi.png",desc:"Clean study fuel."},
  {id:"ikura",name:"Ikura Bowl",price:65,img:"assets/store-ikura.png",desc:"Premium reward bowl."},
  {id:"ramen",name:"Ramen Bowl",price:90,img:"assets/store-ramen.png",desc:"Warm boss-round energy."},
  {id:"skewers",name:"Festival Skewers",price:55,img:"assets/store-skewers.png",desc:"Matsuri snack vibes."}
];
let trainerMode="kana",currentQuestion=null,lockAnswer=false,mapLayerMode="regions",lastRewardAt=0;
...
progress.name??="Guest Learner";progress.points??=0;progress.exp??=0;progress.level??=1;progress.yen??=0;progress.streak??=0;progress.lastNameChange??=0;progress.nameLogs??=[];progress.economyHash??="";progress.triedLearning??=false;progress.inventory??={};
...
window.addEventListener("load",()=>{setTimeout(()=>preloader.classList.add("hidden"),900);createMapPins();loadTrainer();animateCounters();if(localStorage.getItem("tasewakaiGuideHidden")!=="true")setTimeout(openGuide,1300);setTimeout(showTrainerCatOnce,2600)});
...
function loadTrainer(){const input=document.getElementById("trainerName");if(input)input.value=progress.name==="Guest Learner"?"":progress.name;const ok=verifyEconomy();updateTrainerUI(ok?"":"Economy integrity warning: local data was edited.");updateLeaderboard();renderNameLogs();updatePlayerHud()}
...
function updatePlayerHud(){const hud=document.getElementById("playerHud");if(!hud)return;hud.classList.toggle("hidden",!hasLearningProgress());if(!hasLearningProgress())return;const next=progress.level*100,fill=Math.min(progress.exp/next*100,100);document.getElementById("hudName").textContent=progress.name||"Guest Learner";document.getElementById("hudLevel").textContent=progress.level;document.getElementById("hudYen").textContent=progress.yen;document.getElementById("hudExpFill").style.width=fill+"%";document.getElementById("hudExpText").textContent=`EXP ${progress.exp}/${next}`;const tier=getHudSakuraTier(),sakuraText=document.getElementById("hudSakuraText");if(sakuraText)sakuraText.textContent=tier==="storm"?"Sakura storm":tier==="heavy"?"Sakura III":tier==="medium"?"Sakura II":"Sakura I";const badge=document.getElementById("hudBadge"),badgeText=document.getElementById("hudBadgeText"),achievement=document.getElementById("hudAchievement");if(badge){badge.classList.toggle("hidden",progress.level<10)}if(achievement){achievement.classList.toggle("unlocked",progress.level>=10)}if(badgeText)badgeText.textContent=progress.level>=10?"Lv.10 verified learner":"Badge at Lv.10";const balance=document.getElementById("storeBalance");if(balance)balance.textContent=progress.yen;renderHudSakuraRain()}
...
function renderStore(){const grid=document.getElementById("storeGrid"),inventory=document.getElementById("inventoryRow"),balance=document.getElementById("storeBalance");if(!grid||!inventory)return;if(balance)balance.textContent=progress.yen;grid.innerHTML=storeItems.map(item=>{const owned=progress.inventory[item.id]||0,canBuy=progress.yen>=item.price;return `<article class="store-item ${canBuy?"":"locked"}"><img src="${item.img}" alt="" /><div><h3>${item.name}</h3><p>${item.desc}</p><strong>${item.price} YEN</strong><small>Owned: ${owned}</small></div><button class="btn ${canBuy?"join-btn":"ghost-btn"} small-btn" onclick="buyStoreItem('${item.id}')" ${canBuy?"":"disabled"}>Buy</button></article>`}).join("");const ownedItems=storeItems.filter(item=>progress.inventory[item.id]);inventory.innerHTML=ownedItems.length?ownedItems.map(item=>`<span><img src="${item.img}" alt="" /> ${item.name} ×${progress.inventory[item.id]}</span>`).join(""):"<small>No snacks yet. Train first, then treat yourself.</small>"}
...
function renderLeaderboard(){const lists=document.querySelectorAll("#leaderboardList");if(!lists.length)return;const html=leaderboard.length===0?"<p>No scores yet. Play the trainer first.</p>":leaderboard.map((item,i)=>`<div class="leaderboard-row"><span><strong>#${i+1}</strong> ${item.name}</span><span>${item.yen||0} YEN · Lv.${item.level}</span></div>`).join("");lists.forEach(list=>list.innerHTML=html)}
...
function mountN5Quiz(){const mount=document.getElementById("n5QuizMount");if(!mount||mount.innerHTML.trim())return;mount.innerHTML=`<div id="jlptQuizView" class="jlpt-quiz-view"><div class="jlpt-hud"><div><p class="mini-label" id="jlptModeLabel">JLPT N5</p><strong id="jlptRoundLabel">Kanji card</strong></div><div class="jlpt-meter"><span>Boss</span><div><b id="jlptBossFill"></b></div></div></div><div class="jlpt-card"><button class="kanji-info-btn" onclick="toggleKanjiInfo()">i</button><div class="quiz-character jlpt-character" id="jlptPrompt">日</div><p class="quiz-question" id="jlptQuestion">Choose the correct meaning.</p><div class="kanji-meta" id="kanjiMeta"></div><div class="kanji-info hidden" id="kanjiInfo"></div></div><div class="answer-grid" id="jlptAnswerGrid"></div><p class="quiz-feedback" id="jlptFeedback">Pick an answer to begin.</p></div><div id="jlptLeaderboardView" class="leaderboard hidden"><p class="mini-label">Local leaderboard</p><h3>Top Learners</h3><div id="leaderboardList"></div></div>`}
