...
function normalizeLearningCard(card){if(!card||typeof card!=="object")return card;repairDataText(card);["q","a","on","kun","reading","example","note","answer"].forEach(key=>{if(typeof card[key]==="string")card[key]=safeLearningText(card[key],key==="q"?"?":"-")});return card}
const cleanKanaDeck=[
...
function updateKanjiMetaBadges(card=currentQuestion){card=normalizeLearningCard(card)||card;const level=getJlptLevelLabel(),strokes=getKanjiStrokeCount(card);const set=(id,text)=>{const el=document.getElementById(id);if(el)el.textContent=text};set("jlptLevelBadge",level);set("jlptStrokeBadge",`${strokes} Strokes`);set("jlptDifficultyBadge",getKanjiDifficulty(card));set("jlptOnReading",safeLearningText(card?.on||"-","-"));set("jlptKunReading",safeLearningText(card?.kun||card?.reading||"-","-"))}
...
function getQuestionPool(){ensureJlptFallbackDeck();let pool;if(trainerMode==="n5")pool=jlptDeck.n5||[];else if(trainerMode==="n4")pool=jlptDeck.n4||[];else if(trainerMode==="n3")pool=jlptDeck.n3||[];else if(trainerMode==="n2")pool=jlptDeck.n2||[];else if(trainerMode==="n1")pool=jlptDeck.n1||[];else if(trainerMode==="review")pool=reviewQueue.length?reviewQueue:(jlptDeck.n5||[]);else if(trainerMode==="words")pool=jlptWordDeck;else if(trainerMode==="kanji")pool=kanjiQuestions;else pool=kanaQuestions;return Array.isArray(pool)?pool.map(normalizeLearningCard).filter(item=>item&&item.q&&item.q!=="?"):[]}
...
function pickJlptCard(pool){pool=pool.map(normalizeLearningCard).filter(card=>card&&card.q&&card.q!=="?");if(!pool.length)return null;const known=new Set(jlptAdventure.knownCards||[]),available=pool.filter(card=>!known.has(`${getJlptLevelLabel()}:${safeLearningText(card.q,"")}`));const source=available.length?available:pool;let picked=source[Math.floor(Math.random()*source.length)];if(source.length>1){let guard=0;while(picked.q===lastJlptQuestionKey&&guard<12){picked=source[Math.floor(Math.random()*source.length)];guard++}}return picked}
